...
`;
  ...
  async function sendMessage(text) {
    ...
    try {
      ...
      // Show Calendly CTA if bot response mentions booking/meeting
      const botResponse = (data.output || '').toLowerCase();
      const meetingPhrases = [
        'check the calendar', 'book a suitable', 'suitable slot', '30-minute call',
        'days and times', 'arrange a time', 'arrange a meeting', 'book a meeting',
        'schedule a call', 'book in a', 'quick call', 'happy to arrange',
        'love to help arrange', 'book an appointment', 'free consultation',
        'times suit you', 'times are best', 'which days', 'what days', 'book a slot'
      ];
      if (meetingPhrases.some(kw => botResponse.includes(kw))) {
        const bookingCta = (cfg().branding?.quickQuestions || [])
          .map(q => typeof q === 'string' ? null : q)
          .find(q => q && q.cta && q.text.toLowerCase().includes('consultation'));
        const bookingUrl = bookingCta ? bookingCta.cta.url : 'https://calendly.com/aleksicboris94/30min';
        const bookingLabel = bookingCta ? bookingCta.cta.label : '📅 Book a free consultation →';
        const link = document.createElement('a');
